...
import { House, GraduationCap, Briefcase, Folder, ContactRound } from 'lucide-react'
  ...
  return (
    <div className='fixed top-0 w-[90%] flex justify-center items-center z-10'>
      ...
      >
        ...
        {isScrolled && (<div className='flex items-center justify-center text-sm'>
          {SECTIONS.map((s) => (
            <button
              key={s.id}
        ...
        {!isScrolled && (<div className='m-5 flex items-center justify-center gap-4 text-2xl'>
          {SOCIALS.map((s) => (
            <a
              key={s.name}
